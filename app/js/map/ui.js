...
function Bottom(selector) {
    ...
    function populateItems(data, multiSelect) {
        ...
        for (const [i, itemData] of data.entries()) {
            ...
            if ((mapType === 'world' || mapType === 'battle') && army_id && enabled) {
                if (name) $('#paneltitle').innerText = name;
                $('#paneltitle').dataset.id = army_id;
            }

            enabled = enabled ? ((mapType === 'world' || mapType === 'battle') && army_id ? 'type_active' : '') : 'type_disabled';
        ...
        switch (component.type) {
            case 'tabs':
                ...
                for (let [index, tab] of component.tabs.entries()) {
                    ...
                    if (index == (component.active_tab ?? 0)) navEl.classList.add('active');
                    ...
                    if (index == (component.active_tab ?? 0)) tabEl.classList.add('active');
...
async function handleClick(url,history=true) {
    ...
    function getSelectedIds() {
        if (ui?.bottom.selected)
        {
            ui.bottom.updateSelected();
    ...
    if (!dev) {
        ...
        if (data.redirect) window.location.href = data.redirect;
            ...
            if (data.hasOwnProperty('army')) ui.bottom.update(data.army,true);
        ...
        if (data.hasOwnProperty('renew_city_map') && (typeof renew_city_map) == 'function' && data.renew_city_map) renew_city_map();
...
function enableRenaming() {
    ...
    function init(elem) {
        ...
        function textValid(string) {
            return true;//валидация на сервере 
        ...
        function sendNewText(string) {
            ...
            fetch(`/ajax`, {
                ...
                body: `do=rename&c=map&id=${id}&o=${object}&name=${string}` 
            }).then((res) => {
                if (res.ok) {
                    res.json().then((res) => {
                        console.log(res);
